package dev.sanmer.auth.crypto

import dev.sanmer.auth.crypto.Crypto.Default.generateSecretKey
import dev.sanmer.auth.crypto.Crypto.Default.randomIv
import dev.sanmer.auth.crypto.Crypto.Default.randomSalt
...
class PasswordKey private constructor(
    ...
    companion object Default {
        private fun CharSequence.toCharArray() = CharArray(length) { get(it) }
        fun new(password: CharSequence) = PasswordKey(password.toCharArray())
    }
}
